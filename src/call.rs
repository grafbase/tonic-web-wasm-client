...
async fn prepare_body(request: Request<BoxBody>) -> Result<Option<JsValue>, Error> {
    let body = request
        ...
        .await
        .transpose()?
        .map(|frame| frame.into_data())
        .transpose()
        // technically this would be the request body but as this is a fork I don't want to change the external API
        .map_err(|_| Error::MissingResponseBody)?;
